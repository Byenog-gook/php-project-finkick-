...
$(document).ready(function () {
  ...
  $.ajax({
    url: "./controller/a_getNotice_board_list.php",
    // post 방식으로 전송
    type: "POST",
    dataType: "json",
    cache: false,
    ...
    success: function (response) {
      ...
      if (response["error"]) {
        ...
        for (i = start; i < start + scale && i < boardCount; i++) {
          ...
          div += "<tbody><tr>";
          div += "<th scope='row'><a href='./index.php?target=a_notice_view&num=" +
            response['result_data'][i]['num'] + "'>" + response['result_data'][i]['num'] + "</a></th>";

          div += "<td><a href='./index.php?target=a_notice_view&num=" +
        ...
        if (totalpage >= 2 && page >= 2) {
          newpage = page - 1;
          div +=
            "<li><a href='./index.php?target=a_notice_list&page=" + newpage + "'>" +
            "◀ 이전" +
            "</a></li>";
        } else {
          div += "<li>&nbsp;</li>";
        ...
        for (i = 1; i <= totalpage; i++) {
          if (page == i) {
            ...
            div +=
              "<li>" +
              "<a href='./index.php?target=a_notice_list&page=" +
              i +
              "'>" +
              i +
              "</a></li>";
