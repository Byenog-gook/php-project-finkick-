...
$(document).ready(function () {
  ...
  accountnum = urlParams.num;

  var accountData = { "accountnum": accountnum };


  $.ajax({
    url: "./controller/getinquire_answer_list.php",
    // post 방식으로 전송
    type: "POST",
    data: accountData,
    dataType: "json",
    ...
    success: function (response) {
      ...
      if (response["error"]) {
        ...
        for (i = start; i < start + scale && i < boardCount; i++) {
            ...
            if(response['result_data'][i]['responseContent']){
          div += "<tr>";
          div += "<td><a href='./index.php?target=inquire_answer_view&num=" +
        ...
        if (totalpage >= 2 && page >= 2) {
          newpage = page - 1;
          div +=
            "<li><a href='./index.php?target=inquire_answer&page=" + newpage + "'>" +
            "◀ 이전" +
            "</a></li>";
        } else {
          div += "<li>&nbsp;</li>";
        ...
        for (i = 1; i <= totalpage; i++) {
          if (page == i) {
            ...
            div +=
              "<li>" +
              "<a href='./index.php?target=inquire_answer&page=" +
              i +
              "'>" +
              i +
              "</a></li>";
...
function BoardList() {
  location.href = "./index.php?target=inquire_answer";
}