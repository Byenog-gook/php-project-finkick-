...
$(document).ready(function () {
  ...
  $.ajax({
    url: "./controller/getNotice_board_list.php",
    // post 방식으로 전송
    type: "POST",
    dataType: "json",
    cache: false,
    ...
    success: function (response) {
      ...
      if (response["error"]) {
        ...
        div += "<tr><th scope='col'>글 번호</th>";
        div += "<th scope='col'>작성자</th>";
        div += "<th scope='col'>제목</th>";
        div += "<th scope='col'>날짜</th></tr></thead>";
        ...
        for (i = start; i < start + scale && i < boardCount; i++) {
          ...
          div += "<tbody><tr>";
          div += "<th scope='row'><a href='./index.php?target=notice_board_view&num=" +
            response['result_data'][i]['num'] + "'>" + response['result_data'][i]['num'] + "</a></th>";

          div += "<td><a href='./index.php?target=notice_board_view&num=" +
            response['result_data'][i]['num'] +
            "'>" + "관리자" + "</a></td>";

            div += "<td><a href='./index.php?target=notice_board_view&num=" +
            response['result_data'][i]['num'] +
            "'>" + response['result_data'][i]['title'] + "</a></td>";

            div += "<td><a href='./index.php?target=notice_board_view&num=" +
            response['result_data'][i]['num'] +
            "'>" + response['result_data'][i]['date'] + "</a></td>";
        ...
        if (totalpage >= 2 && page >= 2) {
          newpage = page - 1;
          div +=
            "<li><a href='./index.php?target=notice_board_list&page=" + newpage + "'>" +
            "◀ 이전" +
            "</a></li>";
        } else {
          div += "<li>&nbsp;</li>";
        ...
        for (i = 1; i <= totalpage; i++) {
          if (page == i) {
            ...
            div +=
              "<li>" +
              "<a href='./index.php?target=notice_board_list&page=" +
              i +
              "'>" +
              i +
              "</a></li>";
...
function BoardWrite() {
  location.href = "./index.php?target=notice_board_write";
}

function BoardList() {
  location.href = "./index.php?target=notice_board_list";
}